...
{
	rc_code_t ir_code; 
	uint8_t key_byte = NO_KEYS;  /* Pressd Multimedia key */
//	uint16_t code;
	uint8_t code;
	int8_t i;
	...
	if(0 != rc_get(&ir_code)) {
		timer_reset();
		RED_LED_TOGGLE();
//		code = (uint16_t)((ir_code.data[2] << 8) | ir_code.data[3]);
		code = ir_code.data;
#if 0
		for(i = 15; i >= 0; i--) {
			RED_LED_ON();
