...
{
	DDRD = (1 << 0); /* PD0 is LED */
    PORTD = (1 << 1); /* activate pull-up for Button(PD1); turn LED ON */
    _delay_us(500);  /* wait for levels to stabilize */
}

#define bootLoaderCondition()   ((PIND & (1 << 1)) == 0)   /* True if jumper is set */
