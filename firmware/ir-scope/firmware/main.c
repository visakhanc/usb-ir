...
/* Interrupt OUT data */
typedef struct {
	uint8_t start;	  /* Set to 1 when no activity for more than say 3 sec, changes to 0 on first egde of IR signal */
	uint8_t id;		  /* Simple rotating counter to check loss of samples */ 
	int16_t duration; /* duration in multiple of Timer resolution (ie 5.33us); -ve indicates LOW LEVEL and +ve HIGH LEVEL */
} report_t;
...
static int16_t duration[MAX_SAMPLES];
